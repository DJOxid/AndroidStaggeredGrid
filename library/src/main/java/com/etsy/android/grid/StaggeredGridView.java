...
public class StaggeredGridView extends ExtendableListView {
    ...
    private boolean firstItemFullWidth = false;
    ...
    public void setFirstItemFullWidth() {
        this.firstItemFullWidth = true;
    }
    ...
    public void setColumnCount(int columnCount) {
        ...
        //fixing unstable changing columns count at onCreate
        mColumnCount = 0;
    ...
    protected void onMeasureChild(final View child, final LayoutParams layoutParams) {
        ...
        if (viewType == ITEM_VIEW_TYPE_HEADER_OR_FOOTER ||
            ...
            if (isShowFirstItemAtFullWidth(position)) {
                ...
                childWidthSpec = MeasureSpec.makeMeasureSpec(mColumnWidth, MeasureSpec.EXACTLY);
            }
            int childHeightSpec;
    ...
    protected void onLayoutChild(final View child,
        ...
        if (isShowFirstItemAtFullWidth(position)) {
            layoutGridFullWidth(child, position, flowDown, childrenLeft, childRight);
        } else if (isHeaderOrFooter(position)) {
            layoutGridHeaderFooter(child, position, flowDown, childrenLeft, childRight);
        } else {
            layoutGridChild(child, position, flowDown, childrenLeft, childRight);
        }
    }

    private void layoutGridHeaderFooter(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childRight) {
    ...
    private void layoutGridFullWidth(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childRight) {
        int gridChildTop;
        int gridChildBottom;
    ...
    protected void onOffsetChild(final View child, final int position,
        ...
        } else if (isShowFirstItemAtFullWidth(position)) {
            offsetGridFullWidthChild(child, position, flowDown, childrenLeft, childTop);
        } else {
    ...
    private void offsetGridFullWidthChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop) {
        int gridChildTop;
        int gridChildBottom;
        ...
        if (child.getWidth() == mColumnWidth) {
            child.layout(childrenLeft, gridChildTop, childrenLeft + mColumnWidth, gridChildTop + childTopMargin + child.getMeasuredHeight());
            onMeasureChild(child, (LayoutParams) child.getLayoutParams());
        }
    ...
    private void offsetGridChild(final View child, final int position, final boolean flowDown, final int childrenLeft, final int childTop) {
        ...
        //for check when child size is wrong
        if (child.getWidth() != mColumnWidth) {
            child.layout(childrenLeft, gridChildTop, childrenLeft + mColumnWidth, gridChildTop + childTopMargin + child.getMeasuredHeight());
            onMeasureChild(child, (LayoutParams) child.getLayoutParams());
        }
    ...
    protected int getLastChildBottom() {
        ...
        if (isHeaderOrFooter(lastPosition) || isShowFirstItemAtFullWidth(lastPosition)) {
    ...
    private int calculateColumnWidth(final int gridWidth) {
        return calculateColumnWidth(gridWidth, mColumnCount);
    }

    private int calculateColumnWidth(final int gridWidth, int columnCount) {
        final int listPadding = getRowPaddingLeft() + getRowPaddingRight();
        return (gridWidth - listPadding - mItemMargin * (columnCount + 1)) / columnCount;
    }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.checkState = listState.checkState;
        ss.checkedItemCount = listState.checkedItemCount;
        ss.checkIdState = listState.checkIdState;
        ss.inActionMode = listState.inActionMode;
